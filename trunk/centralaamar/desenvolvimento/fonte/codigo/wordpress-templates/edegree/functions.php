...
$options = array (
	...
	array(  "name" => "Cor do Tema",
			"desc" => "Escolha a cor do tema.",
            "id" => $shortname."_skin_color",
			"default" => "silver",
            "type" => "skin_color"),
	
	array(  "name" => "Cor do Texto",
			"desc" => "Escolha a cor que pretende utilizar no texto. A cor original é #575757.",
            "id" => $shortname."_font_color",
			"default" => "#575757",
            "type" => "font_color"),
	
	array(  "name" => "Logótipo",
			"desc" => "Gostaria de mostrar o logótipo no cabeçalho?",
            "id" => $shortname."_logo_header",
			"default" => "no",
            "type" => "logo"),
			
	array(  "name" => "Localização do Logótipo",
			"desc" => "Pretende que o logótipo seja alinhado a esquerda, centro ou direita?",
            "id" => $shortname."_logo_location",
			"default" => "left",
            "type" => "logo-location"),
			
	array(  "name" => "Tamanho do Texto",
			"desc" => "Escolha o tamanho do texto vai ser utilizado no seu site.",
            "id" => $shortname."_font_size",
			"default" => "12px",
            "type" => "font_size"),
			
	array(  "name" => "Formulário de Pesquisa",
			"desc" => "Gostaria de mostrar um formulário de pesquisa no cabeçalho? (yes » sim - no » não)",
            "id" => $shortname."_search_header",
			"default" => "yes",
            "type" => "search"),
			
	array(  "name" => "Barra de Administração / Login",
			"desc" => "Gostaria de mostrar uma barra para login / administrar páginas ou posts no topo do site? (yes » sim - no » não)",
            "id" => $shortname."_user_login",
			"default" => "no",
            "type" => "login"),
	
	array(  "name" => "Socialize Icons",
			"desc" => "Insira o link para o seu Twitter, Facebook, e RSS feeds.<br />Se pretender esconder o icon, escreva \"hide\" no campo.",
	...
	array(  "name" => "Texto de Copyright",
		    "id" => $shortname."_copy_year",
			"desc" => "Insira o texto de Copyright utilizado no footer. (Exemplo: \"Copyright &copy; 2000-".date('Y',time())."\")",
			"default" => "",
            "type" => "copyright"),
...
<h2><?php echo $themename; ?> Opções
	...
	if(stristr($_GET['page'],'tbf-design.php')) {
		...
		echo '- Página de Aterragem';
	} elseif(stristr($_GET['page'],'tbf-optin.php')) {
		echo '- Email Optin / Newsletter';
	} else {
		echo '- Configuração da Homepage'; }  
	?>
</h2>
<?php
if (isset($_REQUEST['saved'])) echo '<div id="message" class="updated fade"><p><strong>'.$themename.' - Opções Salvas.</strong></p></div>';
?>
...
<div id="side-info-column" class="inner-sidebar">
	<div id='side-sortables' class='meta-box-sortables'>
		<div id="linksubmitdiv" class="postbox " >
			<h3 class='hndle'><span>Opções Actuais</span></h3>
			<div class="inside">
				<div class="submitbox" id="submitlink">
					<div id="minor-publishing">
						...
						<div class="misc-pub-section misc-pub-section-last">
							...
							<?php if(stristr($_GET['page'],'tbf-design.php')) { ?>
									<li>Cor do Tema: <strong><?php echo ucwords(get_option($shortname.'_skin_color')); ?></strong></li>
									<li>Cor do Texto (Hex): <strong><?php echo ucwords(get_option($shortname.'_font_color')); ?></strong></li>
                                    <li>Logótipo: <strong><?php echo ucwords(get_option($shortname.'_logo_header')); ?></strong></li>
									<li><?php if (get_option($shortname.'_logo_header') == "yes") { echo "Logo"; } else { echo "Blog Name"; } ?> Localização: <strong><?php echo ucwords(get_option($shortname.'_logo_location')); ?></strong></li>
							<?php } ?>
							<?php if(stristr($_GET['page'],'tbf-features.php')) { ?>
									<li>Tamanho do Texto: <strong><?php if(get_option($shortname.'_font_size') == "11px") { echo "11px"; } else { echo ucwords(get_option($shortname.'_font_size')); } ?></strong></li>
									<li>Barra de Pesquisa: <strong><?php if(get_option($shortname.'_logo_location') == "middle") { echo "Não"; } else { echo ucwords(get_option($shortname.'_search_header')); } ?></strong></li>
									<li>Barra de Administração: <strong><?php if(get_option($shortname.'_user_login') == "yes") { echo "Sim"; } else { echo ucwords(get_option($shortname.'_user_login')); } ?></strong></li>
							<?php } ?>
							<?php if(stristr($_GET['page'],'tbf-homepage.php')) { ?>
									<li>Numero de Posts: <strong><?php echo get_option('posts_per_page'); ?></strong></li>
                                    <li>Slideshow: <strong><?php echo (get_option($shortname.'_custom_slidespot') == 'yes') ? 'Não' : 'Sim'; ?></strong></li>
							<?php } ?>
							...
							<?php if(stristr($_GET['page'],'tbf-landing.php')) { ?>
									<li>Landing Page Header: <strong><?php if(get_option($shortname.'_landing_header') == "displayed") { echo "Mostrando"; } else { echo ucwords(get_option($shortname.'_landing_header')); } ?></strong></li>
									<li>Landing Page Footer: <strong><?php if(get_option($shortname.'_landing_footer') == "displayed") { echo "Mostrando"; } else { echo ucwords(get_option($shortname.'_landing_footer')); } ?></strong></li>
							<?php } ?>
							</ul>
						</div>
						</div>
					...
					<div id="major-publishing-actions">
						...
						<div id="publishing-action">
							<input name="save" type="submit" class="button-primary" value="Guardar Opções" />    
	...
	if(stristr($_GET['page'],'tbf-design.php')) {
			...
			if(isset($value['type'])) {
				switch ($value['type']) {
							...
							<table>
								<tr>
									<td align="center" style="padding-right: 10px;">
										<input name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>2" type="radio" value="silver"<?php if(get_option($value['id']) == "silver") { echo " checked"; } ?> /> Branco/Cinzento
									</td>
									<td align="center" style="padding-right: 10px;">
										<input name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>3" type="radio" value="red"<?php if(get_option($value['id']) == "red") { echo " checked"; } ?> /> Vermelho
									</td>
									<td align="center" style="padding-right: 10px;">
										<input name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>1" type="radio" value="blue"<?php if(get_option($value['id']) == "blue") { echo " checked"; } ?> /> Azul
									</td>
								</tr>
								<tr>
									<td style="padding-right: 10px;">
										<label for="<?php echo $value['id']; ?>2"><img src="<?php bloginfo('template_url'); ?>/images/admin/screen-silver.png" alt="Branco/Cinzento" /></label>
									</td>
									<td style="padding-right: 10px;">
										<label for="<?php echo $value['id']; ?>3"><img src="<?php bloginfo('template_url'); ?>/images/admin/screen-red.png" alt="Vermelho" /></label>
									</td>
									<td style="padding-right: 10px;">
										<label for="<?php echo $value['id']; ?>1"><img src="<?php bloginfo('template_url'); ?>/images/admin/screen-blue.png" alt="Azul" /></label>
									</td>
								</tr>
							</table>
					...
					<div id="logodiv" class="stuffbox">
						<h3><label for="link_url"><?php echo $value['name']; ?></label></h3>
						<div class="inside">
							<label><input name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" type="radio" value="yes"<?php if(get_option($value['id']) == "yes") { echo " checked"; } ?> />&nbsp;Sim</label>&nbsp;&nbsp;&nbsp;&nbsp;<label><input name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" type="radio" value="no"<?php if(get_option($value['id']) == "no") { echo " checked"; } ?> />&nbsp;Não&nbsp;(Só o nome do meu blog)</label>
							
							<p><small><?php echo $value['desc']; ?></small></p>
							<div id="headerLogo">
								Fazer upload do logótipo: <input type="file" name="logo" id="logo" /> (Altura recomendada: 56px)
		...
		?>
				...
				<div id="footer_image_div" class="stuffbox">
					<h3><label for="link_url">Imagem de Fundo</label></h3>
					<div class="inside">
					<?php $imageURL = get_option($shortname.'_background_image_file');?>
						Link directo para a imagem: <input name="<?php echo $shortname?>_background_image_file" id="<?php echo $shortname?>_background_image_file" type="text" size="50" value="<?php echo htmlentities($imageURL);?>">
						<div id="headerLogo">
							Ou faça o upload da imagem: <input type="file" name="background_image_file" id="background_image_file" />
						</div>
						
						Repetição do Fundo: 
						<?php $bgr = get_option($shortname.'_background_repeat');?>
						
						<input type="radio" size="8" name="<?php echo $shortname;?>_background_repeat" <?php echo ($bgr == 'repeat-x'  ? 'checked="checked"' : '');?> value="repeat-x"  id="repeat-x" ><label for="repeat-x">Horizontal</label> &nbsp;
						<input type="radio" size="8" name="<?php echo $shortname;?>_background_repeat" <?php echo ($bgr == 'repeat-y'  ? 'checked="checked"' : '');?> value="repeat-y"  id="repeat-y" ><label for="repeat-y">Vertical</label> &nbsp;
						...
						<input type="radio" size="8" name="<?php echo $shortname;?>_background_repeat" <?php echo ($bgr == 'no-repeat' ? 'checked="checked"' : '');?> value="no-repeat" id="no-repeat" ><label for="no-repeat">Não Repetir</label>
						<br>
						<span class="colorpicker">
                        	Cor de Fundo: <input type="text" size="8" name="<?php echo $shortname;?>_background_color" id="background_color" value="<?php echo htmlentities($bgc = get_option($shortname.'_background_color'));?>"> <span class="color-sample" style="background-color:<?php echo $bgc;?>;padding:0 10px;">&nbsp;</span><br>
						...
						<?php endif;?>
						<br /><span class="description">Deixe em branco para utilizar as definições originais</span>
					</div>
				</div>
				
				<div id="header_image_div" class="stuffbox">
					<h3><label for="link_url">Imagem do Cabeçalho</label></h3>
					<div class="inside">
					<?php $imageURL = get_option($shortname.'_header_image_file');?>
						Link directo para a imagem: <input name="<?php echo $shortname?>_header_image_file" id="<?php echo $shortname?>_header_image_file" type="text" size="50" value="<?php echo htmlentities($imageURL);?>"> <small>Tamanho recomendado: 900px de altura e largura</small>
						<div id="headerLogo">
							Ou faça o upload da imagem: <input type="file" name="header_image_file" id="header_image_file" />
						</div>
						<?php if ($imageURL):?>
				...
				<div id="footer_image_div" class="stuffbox">
					<h3><label for="link_url">Imagem do Footer</label></h3>
					<div class="inside">
					<?php $imageURL = get_option($shortname.'_footer_image_file');?>
						Link directo para a imagem: <input name="<?php echo $shortname?>_footer_image_file" id="<?php echo $shortname?>_footer_image_file" type="text" size="50" value="<?php echo htmlentities($imageURL);?>"> <small>Tamanho recomendado: 900px x 200px</small>
						<div id="headerLogo">
							Ou faça o upload da imagem: <input type="file" name="footer_image_file" id="footer_image_file" />
						</div>
						<?php if ($imageURL):?>
							<img src="<?php echo $imageURL;?>" style="max-width:600px" />
						<?php endif;?>
